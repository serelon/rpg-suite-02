...
| [[0099-pseudocode-as-encoding]] | idea | conversation | **adopt — scoped to reading 1** (reappraised 2026-06-15; split the readings — see [[0118-encoding-by-data-type]]). Format prompts/lore as pseudocode. Reading 1 lore-as-data (relational, category 3) **adopt** — committed direction, still `maturity/experimental` (untested in own workflow, came from SillyTavern). Reading 2 logic-as-control-flow **stays gated** — railroad risk is in encoding *behavior*; campaign trial is the gate for this half specifically. Structured records (sheets) are category 2, already solved. Early win: human auditability. |
