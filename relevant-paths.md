...
|------|------|--------|----------|
| [[0128-provenance-backlinks-at-update-granularity]] | principle | solorpg | **Every derived datum backlinks to its source *updates*** (`character-A ← [session03/45], [session03/68]`) so you retrieve exact source context without rereading. **The address space = 0124's update-chunks** — chunking *mints* the anchors; chunk→postprocess(emit backlinked)→review(resolve) is one mechanism across three loop stages. Live skill confirms the gap: chunks are addressable but no output backlinks them, so v1's "get source back" = *reread chunks*; backlinks make it *targeted retrieval*. Keystone under the review-GUI/audit (0127 visibility) and re-derivation (0094); concrete form of 0111/0076/0103/0102; enforces SSoT-by-reference (T8). **Unevaluated.** |
| [[0127-postprocess-as-controlled-mess]] | pattern | solorpg | **`[postprocess]` is an irreducible mess — build the thing that *controls* it, not a clean pipeline.** Owner thesis: it'll stay messy; it's many small tasks in one workflow (its own loop). Three control pillars: **orchestration / steering-override / visibility**. Next-gen move: **orchestrator skill + atomic step-skills** — the live v1 skill's 6-phase spine already *is* the orchestrator; lift each Phase-4 output (summary/savefile-merge/memories/locations/characters+core-guardrail/campaign-specific) **+ its existing guide + strict thinking** into a per-thing skill. (v2 team/Lorekeeper variant already deprecated — context-window 0031 workaround retired.) Visibility rides on [[0128]]. Kin to 0036/0023/E:\rpg-pipeline/0075. **Unevaluated.** |
| [[0126-reinterpretation-sands-off-load-bearing-edges]] | observation | solorpg/conversation | **The designed two-mode capture (summary-of-events + verbatim cut'n'paste copy) collapsed to one** — everything gets reinterpreted/summarized/compacted, so verbatim effectively doesn't exist. Lossy *in the wrong place*: pattern-match details get squished ([[0008]] form-is-the-lesson), and **load-bearing edges get sanded off** (horror scenes smoothed until the horror is *gone*). Root cause = **summarization-as-default**; every read-then-rewrite pass silently re-voices. Generalizes [[0054]] (adds the mechanism); the prose form of [[0073]] append-don't-rewrite. Pairs with [[0125]] — never-captured vs captured-then-sanded. Fix likely: hard copy/summarize separation + route verbatim *around* lossy stages. **Unevaluated.** |
| [[0125-texture-has-no-capture-path]] | observation | solorpg/conversation | **The memory system can't capture low-weight texture** — the standing, minor-but-memorable details of a world (how a uniform looks, a dead-drop procedure) — **and mechanical texture**: the *rendering contract* for an object (how a specific weapon should be *written*, vs leaving the GM to re-improvise = drift; the prose layer of [[0106]] beyond characters; needed **in proportion to how invented the thing is** — contemporary objects have a stable shared referent, fantasy/sci-fi don't → drift). A **prompting gap, not storage**: extraction asks "what mattered *most*?" so texture (load-bearing for *immersion/consistency*, not *plot*) falls through; the `sensory` type is for emotional *moments*, not standing *facts*. [[0030]]'s "texture evicted to memories" valve **leaks** — design says memories hold it, prompt doesn't pull it (live failure, not missing-by-design). Owner: texture-bank [[0055]] link is *"bigger chunks of the same thing"* — memory-texture = larger/more-specific chunks, bank entries smaller; **likely one granularity spectrum, not two artifacts** (a design-time call). **Unevaluated.** |
